...
CREATE TABLE IF NOT EXISTS `ap_content_category` (
  ...
  `approval_flow_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '关联审核流程',
  `status` TINYINT NOT NULL DEFAULT 1,
  `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `ap_content` (
  ...
  `status` VARCHAR(16) NOT NULL DEFAULT 'PUBLISHED' COMMENT 'PENDING/PUBLISHED/REJECTED/OFFLINE',
  `author_user_id` BIGINT UNSIGNED DEFAULT NULL,
  `author_role` VARCHAR(16) DEFAULT NULL COMMENT 'PUBLIC/LAW/RESCUE',
  `target_org_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '审核机构/目标机构',
