...
- `name`：分类名称
- `sort_no`：排序号
- `approval_flow_id`：关联审核流程
- `status`：状态
- `created_at`：创建时间
- `updated_at`：更新时间

说明：分类可绑定审核流程，流程节点支持管理员/执法机构/救助机构组合，机构节点可指定目标机构。
...
- `author_user_id`：发布人用户ID
- `author_role`：发布人角色（PUBLIC/LAW/RESCUE）
- `target_org_id`：目标机构ID（审核/推送机构）
- `published_at`：发布时间
- `created_at`：创建时间
- `updated_at`：更新时间
