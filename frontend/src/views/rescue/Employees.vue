...
import {
  listRescueEmployees,
  createRescueEmployee,
  updateRescueEmployee,
  deleteRescueEmployee
} from "@/api";

export default {
  name: "RescueEmployees",
  data() {
    return {
      list: [],
      loading: false,
  ...
  methods: {
    async fetchList() {
      this.loading = true;
      try {
        const resp = await listRescueEmployees();
        this.list = resp.data || [];
      } finally {
        this.loading = false;
      }
    ...
    async submit() {
      if (this.editingId) {
        await updateRescueEmployee(this.editingId, {
          nickname: this.form.nickname,
          password: this.form.password,
          status: this.form.status
        });
      } else {
        await createRescueEmployee({
    ...
    async remove(row) {
      await deleteRescueEmployee(row.id);
      this.fetchList();
    }
  }
};
